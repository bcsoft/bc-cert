		...
		success:function(json){
			if(json.success = false){
				...
				for(var i = length-1 ; i >= 0 ; i--){
					var HiddenDate = "<input type='hidden' class='ignore' name='code' value='"+lists[i].code+"'></input><input type='hidden' class='ignore' name='typeCode' value='"+lists[i].typeCode+"'>" +
								"</input><input type='hidden' class='ignore' name='pid' value='"+option.pid+"'></input><input type='hidden' class='ignore' name='ver' value='"+lists[i].version+"'></input>"+
								"</input><input type='hidden' class='ignore' name='uid' value='"+lists[i].uid+"'></input><input type='hidden' class='ignore' name='fid' value='"+lists[i].fid+"'></input>"+
								"</input><input type='hidden' class='ignore' name='readonly' value='"+option.readonly+"'><input type='hidden' class='ignore' name='name' value='"+lists[i].name+"'></input>"+
								"</input><input type='hidden' class='ignore' name='pname' value='"+option.pname+"'><input type='hidden' class='ignore' name='uuid' value='"+option.uid+"'></input>"+
								"</input><input type='hidden' class='ignore' name='version' value='"+lists[i].version+"'><input type='hidden' class='ignore' name='role' value='"+option.role+"'></input>"+
								"</input><input type='hidden' class='ignore' name='tpl' value='"+lists[i].tpl+"'></input><input type='hidden' class='ignore' name='subject' value='"+lists[i].subject+"'></input>";
...
bc.cert.nestedGrid.printORReadAll = function(option){
	...
	//console.log(isSelectAll.checked == true);
	var certs = [];
				...
				if(isGetDetails){ //需要获取detail，这将覆盖住form-field的数据
					...
					if(details.length > 0){  //有明细信息
						for(var i = 0 ; i < details.length ; i++){  //遍历获取全部的明细信息，并封装成{name:"" ,value:"", type:""}格式
							...
							var page_name = {name:"attach_name_"+details[i].page_no,value:details[i].attach_name,type:"string"};						
